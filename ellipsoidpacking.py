...
system = Pack(molecules=chain, density=0.01*u.Unit("nm**-3"), packing_expand_factor=5)
...
ellipsoid_sim=Simulation(initial_state=rigid_frame, forcefield=forces.hoomd_forces, 
                         ...
                         gsd_file_name="ellipsoids1.gsd", log_write_freq=int(1e4), log_file_name="log-ellipsoids1")
...
ellipsoid_sim.run_update_volume(final_box_lengths=target_box, kT=.001, n_steps=1e5,tau_kt=100*ellipsoid_sim.dt,period=10,thermalize_particles=True)
print("shrink finished")
ellipsoid_sim.run_NVT(n_steps=1e6, kT=.01, tau_kt=10*ellipsoid_sim.dt)
...
ellipsoid_gsd(gsd_file="ellipsoids1.gsd",new_file="ovito-ellipsoid.gsd",lpar=1.0,lperp=0.5, ellipsoid_types="R")
