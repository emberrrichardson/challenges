...
forces = EllipsoidForcefield(epsilon=100, lpar=1.0, lperp=0.5, r_cut=2.5, bond_k=0, bond_r0=0)
#define the sim
#change file name
ellipsoid_sim=Simulation(initial_state=rigid_frame, forcefield=forces.hoomd_forces, 
                         constraint=d_constraint, dt=0.0001, gsd_write_freq=int(1e4), 
                         gsd_file_name="elipschal.gsd", log_write_freq=int(1e4), log_file_name="log-elipschal")
#CAN CHANGE= density, kT(keep consistent in shrink + run)

target_box = get_target_box_number_density(density=.2*u.Unit("nm**-3"),n_beads=128)
ellipsoid_sim.run_update_volume(final_box_lengths=target_box, kT=2, n_steps=1e5,tau_kt=5*ellipsoid_sim.dt,period=10,
                                thermalize_particles=True)
print("shrink finished")
print(ellipsoid_sim.box_lengths_reduced)
ellipsoid_sim.run_NVT(n_steps=1e5, kT=2, tau_kt=5*ellipsoid_sim.dt, thermalize_particles=False)
ellipsoid_sim.run_update_volume(final_box_lengths=target_box, 
                                kT=1, n_steps=1e5,
                                tau_kt=5*ellipsoid_sim.dt,period=10,thermalize_particles=False)


# In[ ]:


ellipsoid_sim.run_NVT(n_steps=1e6, kT=1, tau_kt=5*ellipsoid_sim.dt)
